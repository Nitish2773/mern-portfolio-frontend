...
export default function CrudManager({ endpoint, readOnly, single, headers, FormComponent }) {
  ...
  return (
    <div className="space-y-4">
      {message.text && (
        <div className={`p-3 rounded-xl font-medium ${message.type === "success" ? "bg-green-500" : "bg-red-500"} text-white`}>
          {message.text}
        </div>
      )}
      ...
      {editing && FormComponent && (
        <Modal isOpen={!!editing} onClose={() => setEditing(null)}>
          <div className="flex flex-col flex-1">
            <FormComponent data={editing} onSave={handleSave} onCancel={() => setEditing(null)} />
          </div>
        </Modal>
      )}
    </div>
